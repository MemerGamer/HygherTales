...
function appWithDownload(cf: CurseForgeClient) {
  const app = new Hono().route("/v1/download", createDownloadRoutes(cf));
...
describe("GET /v1/download/:projectId/:fileId", () => {
  test("returns 400 VALIDATION_ERROR for invalid projectId", async () => {
    const app = appWithDownload(mockClient());
    const res = await app.request("http://localhost/v1/download/0/123");
  ...
  test("returns 400 VALIDATION_ERROR for invalid fileId", async () => {
    const app = appWithDownload(mockClient());
    const res = await app.request("http://localhost/v1/download/123/0");
    expect(res.status).toBe(400);
    const body = await res.json();
  ...
  test("returns 400 for non-numeric projectId", async () => {
    const app = appWithDownload(mockClient());
    const res = await app.request("http://localhost/v1/download/abc/456");
    expect(res.status).toBe(400);
  });

  test("returns 503 DOWNLOAD_NOT_AVAILABLE when URL is null", async () => {
    const app = appWithDownload(mockClient());
    const res = await app.request("http://localhost/v1/download/306612/4500000");
  ...
  test("returns 200 and JSON { url } when URL is available", async () => {
    ...
    const app = appWithDownload(client);
    const res = await app.request("http://localhost/v1/download/306612/4500000");
